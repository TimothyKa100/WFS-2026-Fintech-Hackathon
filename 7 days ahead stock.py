...
SECTOR_MAP_PATH = os.path.join(BASE_DIR, "sector_map.csv")


class Stock4DayModel:
    def __init__(self):
        self.start_datetime = datetime.datetime(2021, 3, 1)
        self.forecast_horizon_days = int(os.getenv("HORIZON_DAYS", "7"))
        self.data_dir = self._resolve_data_dir()
        self.device = "cuda"
        self.train_seeds = [42, 2026, 3407]
        self.n_splits = 5
        self.market_neutral_strength = 0.25
        self.output_tag = f"stock_{self.forecast_horizon_days}d"
        self.symbol_cluster_map = {}
        self.enable_cluster_rank_boost = os.getenv("ENABLE_CLUSTER_RANK_BOOST", "0") == "1"
        self.holdout_days = int(os.getenv("HOLDOUT_DAYS", "252"))
        self.select_on_holdout = os.getenv("SELECT_ON_HOLDOUT", "0") == "1"
        self.feature_prune_ratio = float(os.getenv("FEATURE_PRUNE_RATIO", "0.55"))
        self.liquid_top_n = int(os.getenv("LIQUID_TOP_N", "220"))
    ...
    def _build_symbol_clusters(self, feature_per_symbol, max_clusters=12):
        descriptor_rows = {}
        for symbol, feat_df in feature_per_symbol.items():
            row = {
                "ret_1d_mean": float(feat_df["ret_1d"].mean(skipna=True)),
                "ret_1d_std": float(feat_df["ret_1d"].std(skipna=True)),
                "mom_20d_mean": float(feat_df["mom_20d"].mean(skipna=True)),
                "vol_20d_mean": float(feat_df["vol_20d"].mean(skipna=True)),
                "atr_pct_14_mean": float(feat_df["atr_pct_14"].mean(skipna=True)),
                "range_pct_mean": float(feat_df["range_pct"].mean(skipna=True)),
                "vol_z20_std": float(feat_df["vol_z20"].std(skipna=True)),
            }
            descriptor_rows[symbol] = row

        desc_df = pd.DataFrame.from_dict(descriptor_rows, orient="index")
        desc_df = desc_df.replace([np.inf, -np.inf], np.nan).fillna(desc_df.median())
        desc_z = (desc_df - desc_df.mean()) / desc_df.std().replace(0, np.nan)
        desc_z = desc_z.replace([np.inf, -np.inf], np.nan).fillna(0.0)

        n_clusters = min(max_clusters, max(3, len(desc_z) // 20))
        km = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
        labels = km.fit_predict(desc_z.values)

        self.symbol_cluster_map = {
            symbol: int(label) for symbol, label in zip(desc_z.index.tolist(), labels)
        }
        print(f"Built pseudo-sector clusters: n_clusters={n_clusters}")

    def _load_sector_map(self, symbols):
        if not os.path.isfile(self.sector_map_path):
            return {}

        try:
            sector_df = pd.read_csv(self.sector_map_path)
            col_map = {c.lower(): c for c in sector_df.columns}
            if "symbol" not in col_map or "sector" not in col_map:
                print("sector_map.csv found but missing required columns: symbol, sector")
                return {}

            symbol_col = col_map["symbol"]
            sector_col = col_map["sector"]
            sector_df = sector_df[[symbol_col, sector_col]].copy()
            ...
            sector_df = sector_df[sector_df["sector"].notna() & (sector_df["sector"] != "")]

            sector_map = dict(zip(sector_df["symbol"], sector_df["sector"]))
            available_map = {s: sector_map[s] for s in symbols if s in sector_map}

            coverage = len(available_map) / max(1, len(symbols))
            print(
                f"Loaded sector_map.csv with {len(available_map)}/{len(symbols)} symbol coverage "
                f"({coverage:.1%})"
            )
            return available_map
        except Exception as exc:
            print(f"Failed to read sector_map.csv: {exc}")
            return {}

    def _standardize_ohlcv(self, df):
        rename_map = {
            "timestamp": "timestamp",
            "Timestamp": "timestamp",
            "date": "timestamp",
        ...
        # Corporate-action consistency: if adjusted close exists, rescale OHLC.
    ...
    def _compute_symbol_features(self, df):
        ...
        vol_10d = ret_1d.rolling(10, min_periods=5).std()
        vol_20d = ret_1d.rolling(20, min_periods=10).std()
        vol_60d = ret_1d.rolling(60, min_periods=20).std()

        downside_vol_20 = ret_1d.clip(upper=0).rolling(20, min_periods=10).std()
        upside_vol_20 = ret_1d.clip(lower=0).rolling(20, min_periods=10).std()
        ...
        feature_map = {
            ...
            "mom_10d": mom_10d,
            "mom_20d": mom_20d,
            "mom_accel_5_20": mom_5d - mom_20d,
            "vol_10d": vol_10d,
            "vol_20d": vol_20d,
            "vol_60d": vol_60d,
            "vol_term_10_60": vol_10d / vol_60d.replace(0, np.nan) - 1,
            ...
            "close_vs_sma20": close / sma20.replace(0, np.nan) - 1,
            "close_vs_sma50": close / sma50.replace(0, np.nan) - 1,
        ...
        raw_target = np.log(
            close.shift(-self.forecast_horizon_days) / close.replace(0, np.nan)
        )
    ...
    def load_panel_data(self):
        ...
        if self.enable_cluster_rank_boost:
            self._build_symbol_clusters(feature_per_symbol)
        ...
        sector_map = self._load_sector_map(all_symbols)
        ...
        # Blend raw target with sector-neutral (if available) then market-neutral residual target.
        market_component = raw_target.mean(axis=1)
        df_target = raw_target.copy()
        ...
        if sector_map:
            ...
            for _, sector_symbols in sector_to_symbols.items():
                if len(sector_symbols) <= 1:
                    continue
                sector_mean = df_target[sector_symbols].mean(axis=1)
                df_target.loc[:, sector_symbols] = df_target[sector_symbols].sub(
                    sector_mean, axis=0
                )

            uncovered = [s for s in all_symbols if s not in sector_map]
            if uncovered:
                df_target.loc[:, uncovered] = df_target[uncovered].sub(
                    self.market_neutral_strength * market_component,
                    axis=0,
                )
            print("Target mode: sector-neutral residual (with market fallback for uncovered symbols)")
        else:
            df_target = raw_target.sub(self.market_neutral_strength * market_component, axis=0)
            print(
                f"Target mode: blended market-neutral {self.forecast_horizon_days}-day log return "
                f"(strength={self.market_neutral_strength})"
            )

        # Build both time-series normalized and cross-sectional normalized features.
        normalized_factors = {}
    ...
    @staticmethod
    def _to_relevance_labels(y_series, n_bins=10):
        rank_pct = y_series.groupby(level=0).rank(method="average", pct=True)
        rel = np.floor(np.clip(rank_pct.values * n_bins, 0, n_bins - 1)).astype(np.int32)
        return rel
    ...
    def train_and_predict(self, factor_dfs, df_target):
        ...
        min_feature_count = max(8, int(0.7 * len(feature_names)))
        ...
        holdout_active = self.holdout_days > 0 and len(uniq_times) > (self.holdout_days + 300)
        if holdout_active:
            ...
            holdout_times = np.array([], dtype=uniq_times.dtype)
        ...
        # Feature pruning using early CV history only (reduces overfit/lookahead).
        feature_names = list(X_cv.columns)
        feature_select_cut = max(120, int(len(uniq_times_cv) * 0.60))
        ...
        keep_n_features = max(16, int(len(feature_names) * self.feature_prune_ratio))
        ...
        gap = max(self.forecast_horizon_days, 7)
        ...
        for fold, (train_t_idx, val_t_idx) in enumerate(tscv.split(uniq_times_cv), start=1):
            ...
            y_train_rel = self._to_relevance_labels(y_train, n_bins=10)
            y_val_rel = self._to_relevance_labels(y_val, n_bins=10)

            d_val_predict = xgb.DMatrix(X_val)
            seed_models = []
            val_pred = np.zeros(len(X_val), dtype=np.float32)

            group_train = X_train.groupby(level=0).size().to_numpy(dtype=np.uint32)
            group_val = X_val.groupby(level=0).size().to_numpy(dtype=np.uint32)

            for seed in self.train_seeds:
                params = dict(base_params)
                params["random_state"] = seed

                d_train = xgb.DMatrix(X_train, label=y_train_rel)
                d_val = xgb.DMatrix(X_val, label=y_val_rel)
                d_train.set_group(group_train)
                d_val.set_group(group_val)
            ...
            if d_holdout is not None and y_holdout is not None:
                ...
                fold_holdout_score = self.weighted_spearmanr(y_holdout, holdout_pred)
                fold_holdout_scores.append(fold_holdout_score)
                print(f"Fold {fold} - Holdout Spearman {fold_holdout_score:.4f}")
            else:
                fold_holdout_scores.append(np.nan)

        if not fold_model_groups:
            raise RuntimeError("No fold model was trained")

        keep_n = max(1, len(fold_model_groups) - 1)
        if holdout_active and self.select_on_holdout and np.isfinite(np.nanmean(fold_holdout_scores)):
            selection_source = np.array(fold_holdout_scores, dtype=np.float64)
            print("Fold selection metric: holdout Spearman")
        else:
            selection_source = np.array(fold_scores, dtype=np.float64)
            print("Fold selection metric: CV Spearman")

        best_idx = np.argsort(selection_source)[::-1][:keep_n]

        selected_scores = np.array([selection_source[i] for i in best_idx], dtype=np.float64)
        selected_maes = np.array([fold_maes[i] for i in best_idx], dtype=np.float64)

        if np.nanmean(selected_scores) < 0.02:
            inv_mae = 1.0 / np.clip(selected_maes, 1e-8, None)
            fold_weights = inv_mae / inv_mae.sum()
            print("Fold weighting mode: inverse MAE (Spearman weak/noisy)")
        else:
            shifted = selected_scores - selected_scores.min() + 1e-6
            fold_weights = shifted / shifted.sum()
            print("Fold weighting mode: shifted Spearman")
        ...
        print(f"Fold Spearman weights: {fold_weights.tolist()}")

        # Strict out-of-fold metrics (no in-sample contamination).
        ...
        if self.enable_cluster_rank_boost:
            pred_df["predict_raw"] = y_pred

            rank_signal = pred_df.groupby(level=0)["predict_raw"].rank(pct=True) - 0.5
            cs_std = pred_df.groupby(level=0)["predict_raw"].transform("std").replace(0, np.nan).fillna(0.0)
            rank_scaled = rank_signal * cs_std * 2.0

            idx_df = pd.DataFrame(index=pred_df.index)
            idx_df["datetime"] = pred_df.index.get_level_values(0)
            idx_df["symbol"] = pred_df.index.get_level_values(1)
            idx_df["cluster"] = idx_df["symbol"].map(self.symbol_cluster_map).fillna(-1).astype(int)
            idx_df["pred"] = pred_df["predict_raw"].values

            cluster_mean = idx_df.groupby(["datetime", "cluster"])["pred"].transform("mean")
            idx_df["pred"] = idx_df["pred"] - cluster_mean
            market_mean = idx_df.groupby("datetime")["pred"].transform("mean")
            neutral_pred = idx_df["pred"] - market_mean

            pred_df["predict_return"] = (
                0.75 * pred_df["predict_raw"]
                + 0.20 * neutral_pred.values
                + 0.05 * rank_scaled.values
            )
            pred_df = pred_df.drop(columns=["predict_raw"])
            print("Applied optional cluster/rank alpha booster (ENABLE_CLUSTER_RANK_BOOST=1)")
    ...
    def run(self):
        ...
        print(
            f"Training on {len(all_symbols)} symbols for {self.forecast_horizon_days}-day ahead target"
        )
