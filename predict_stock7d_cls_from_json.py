...
def main():
    parser = argparse.ArgumentParser(
        description="Inference for models trained by 7 days ahead stock copy.py"
    )
    ...
    factor_dfs = factor_dfs_from_load_panel_data(
        module_path=str(Path("7 days ahead stock copy.py").resolve()),
        class_name="Stock7DayClassificationModel",
        module_name="stock7d_cls_module",
        factor_position=1,
    )
    ...
    if pred.ndim != 2:
        raise RuntimeError("Expected 3-class probabilities for stock 7-day classifier")

    write_3class_output(pred, x.index, args.out)
