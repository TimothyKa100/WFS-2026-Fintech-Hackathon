...
class OptimizedModel:
    ...
    def __init__(self):
        ...
        self.output_start_datetime = self.start_datetime
        self.scaler = StandardScaler()
        self.processing_device = PROCESSING_DEVICE
        self.training_device = TRAIN_DEVICE
        self.cache_dir = TRAIN_CACHE_DIR
        self.label_horizon_bars = 4 * 6  # 6h horizon on 15m bars
        self.cv_embargo_bars = 4  # extra embargo beyond purge
        self.train_seeds = [42, 2026, 3407]
        self.market_symbols = ["BTCUSDT", "ETHUSDT"]
        self.use_market_neutral_target = True
    ...
    def spearman_ic_mean(self, y_true, y_pred):
        ...
        ic_series = eval_df.groupby(level=0).apply(
    ...
    def regression_metrics(self, y_true, y_pred):
        ...
        y_true_series = pd.Series(y_true_values)
        q05 = y_true_series.quantile(0.05)
        q95 = y_true_series.quantile(0.95)
        tail_mask = (y_true_values < q05) | (y_true_values > q95)
        tail_mae = float(np.nanmean(np.abs(err[tail_mask]))) if tail_mask.any() else np.nan

        return mae, rmse, tail_mae
    ...
    def train(self, df_target, factor_dfs):
        ...
        MAX_BINS = 128
        PURGE_BARS = self.label_horizon_bars
        EMBARGO_BARS = self.cv_embargo_bars
        GAP = max(4 * 24 * 7, PURGE_BARS + EMBARGO_BARS)
        ...
        for fold, (train_t_idx, val_t_idx) in enumerate(tscv.split(uniq_times), start=1):  # type: ignore
            ...
            val_start_idx = val_t_idx[0]
            purged_train_t_idx = train_t_idx[train_t_idx < (val_start_idx - PURGE_BARS)]
            if len(purged_train_t_idx) == 0:
                print(f"Fold {fold} skipped: no train samples after purging")
                continue

            train_times = uniq_times[purged_train_t_idx]
            ...
            seed_models = []
            y_pred_val = np.zeros(len(y_val), dtype=np.float32)
            ...
            for seed in self.train_seeds:
                ...
                d_val_ref = xgb.QuantileDMatrix(X_val, y_val, max_bin=MAX_BINS, ref=d_train)

                model = xgb.train(
                    params=params,
                    dtrain=d_train,
                    evals=[(d_train, "train"), (d_val_ref, "val")],
            ...
            y_pred_val /= len(seed_models)

            score = self.weighted_spearmanr(y_val, y_pred_val)
            ic_mean = self.spearman_ic_mean(y_val, y_pred_val)
            mae, rmse, tail_mae = self.regression_metrics(y_val, y_pred_val)

            fold_model_groups.append(seed_models)
            fold_maes.append(mae)
            print(
                f"Fold {fold} (seeds={len(seed_models)}) - Weighted Spearman {score:.4f}, "
                f"IC mean {ic_mean:.4f}, MAE {mae:.6f}, RMSE {rmse:.6f}, Tail MAE {tail_mae:.6f}"
            )
        ...
        # Keep all but the worst 2 folds by validation MAE (lower is better).
        if len(fold_model_groups) == 0:
            print("No valid folds were trained. Aborting.")
            return

        keep_n = max(1, len(fold_model_groups) - 2)
        best_models_idx = np.argsort(np.array(fold_maes))[:keep_n]

        selected_maes = np.array([fold_maes[idx] for idx in best_models_idx], dtype=np.float64)
        fold_weights = 1.0 / np.clip(selected_maes, 1e-8, None)
        fold_weights = fold_weights / fold_weights.sum()

        model_weight_pairs = []
        for local_idx, fold_idx in enumerate(best_models_idx):
            seed_models = fold_model_groups[fold_idx]
            per_model_weight = fold_weights[local_idx] / max(1, len(seed_models))
            for model in seed_models:
                model_weight_pairs.append((model, per_model_weight))

        best_models = [model for model, _ in model_weight_pairs]
        model_weights = np.array([w for _, w in model_weight_pairs], dtype=np.float32)
        print(f"Selected fold indices: {list(best_models_idx)}")
        print(f"Fold weights (inv MAE): {fold_weights.tolist()}")
        ...
        for i, start in enumerate(range(0, n, BATCH_SIZE)):
            ...
            if i == SHAP_BATCH_IDX:
                ...
                for model, model_weight in zip(best_models, model_weights):
                    ...
                    y_pred[start:end] += (
                        model_shap.sum(axis=1, dtype=np.float32) * model_weight
                    )
                    shap_values += model_shap * model_weight
        ...
        if isinstance(data.index, pd.MultiIndex):
            # Cross-sectional clipping per timestamp to reduce extreme outliers.
            data["y_pred"] = data.groupby(level=0)["y_pred"].transform(
                lambda s: s.clip(s.quantile(0.01), s.quantile(0.99))
            )
            # Smooth each symbol independently (avoid cross-symbol leakage).
            data["y_pred"] = data["y_pred"].groupby(level=1).transform(
                lambda s: s.ewm(span=3).mean()
            )
        else:
            data["y_pred"] = data["y_pred"].ewm(span=3).mean()

        # Global clipping anchored to target scale to suppress extreme startup values.
        target_lo = float(data["target"].quantile(0.005))
        target_hi = float(data["target"].quantile(0.995))
        data["y_pred"] = data["y_pred"].clip(target_lo, target_hi)
        print(f"Applied global prediction clip to target quantiles: [{target_lo:.6f}, {target_hi:.6f}]")
        ...
        overall_ic = self.spearman_ic_mean(data["target"], data["y_pred"])
        overall_mae, overall_rmse, overall_tail_mae = self.regression_metrics(
            data["target"], data["y_pred"]
        )
        print(
            f"Overall IC mean {overall_ic:.4f}, MAE {overall_mae:.6f}, "
            f"RMSE {overall_rmse:.6f}, Tail MAE {overall_tail_mae:.6f}"
        )
        ...
        if OUTPUT_CSV:
            try:
                ...
                df_submit = df_submit[df_submit["datetime"] >= self.output_start_datetime]
                df_submit["id"] = (
                ...
                df_submit_competion = df_submit_competion.drop_duplicates(
                    subset=["id"], keep="last"
                )
                ...
                df_check = df_check[df_check["datetime"] >= self.output_start_datetime]
                df_check["id"] = (
    ...
    def run(self):
        ...
        if not use_derived_cache:
            ...
            derived_dfs["realized_vol_1h"] = winsorize(
                returns.rolling(windows_1h, min_periods=windows_1h // 2).std()
            )
            derived_dfs["realized_vol_6h"] = winsorize(
                returns.rolling(windows_6h, min_periods=windows_6h // 2).std()
            )
            derived_dfs["realized_vol_24h"] = winsorize(
                returns.rolling(windows_1d, min_periods=windows_1d // 2).std()
            )
            derived_dfs["vol_term_1h_24h"] = winsorize(
                (
                    derived_dfs["realized_vol_1h"]
                    / derived_dfs["realized_vol_24h"].replace(0, np.nan)
                )
                - 1
            )
            derived_dfs["vol_of_vol_24h"] = winsorize(
                derived_dfs["realized_vol_6h"]
                .pct_change()
                .rolling(windows_1d, min_periods=windows_1d // 2)
                .std()
            )
        ...
        VOL_FEATURE_SET = "extended"  # "core" or "extended"
        ...
        raw_factors_extended = [*raw_factors_core, "mfi"]
        derived_factors_extended = [
            *derived_factors_core,
            "hour_sin",
            "hour_cos",
            "dow_sin",
        ...
        if VOL_FEATURE_SET == "core":
            raw_factors = raw_factors_core
            derived_factors = derived_factors_core
        else:
            raw_factors = raw_factors_extended
            derived_factors = derived_factors_extended

        print(
            f"Using VOL_FEATURE_SET={VOL_FEATURE_SET} "
            f"(raw={len(raw_factors)}, derived={len(derived_factors)})"
        )
        ...
        for key in raw_factors:
            ...
            elif key in derived_dfs:
                features_dfs_to_use[key] = derived_dfs[key]
            else:
                print(f"Warning: {key} not found in raw or derived data, skipping.")

        for key in derived_factors:
            if key in derived_dfs:
                features_dfs_to_use[key] = derived_dfs[key]
            elif key in raw_dfs:
                features_dfs_to_use[key] = raw_dfs[key]
            else:
        ...
        market_inds_extended = [*market_inds_core, "vwap_deviation_rz"]
        market_inds = (
            market_inds_core if VOL_FEATURE_SET == "core" else market_inds_extended
        )

        for factor in market_inds:
            if factor not in features_dfs_to_use:
                print(f"Missing factor {factor}, cannot proceed.")
                return
        ...
        min_history_bars = max(SLOW_Z_WIN, windows_60d)
        if len(time_index) > min_history_bars:
            warmup_start = pd.Timestamp(time_index[min_history_bars]).to_pydatetime()
            self.output_start_datetime = max(self.start_datetime, warmup_start)
        else:
            self.output_start_datetime = self.start_datetime

        print(
            f"Prediction output starts at {self.output_start_datetime} "
            f"(warmup bars={min_history_bars})"
        )

        keep_slice = slice(self.output_start_datetime, None)
        ...
        # Target: forward 6-hour volatility change based on 15-min VWAP returns.
        # vol_6h_t = std(ret_15m, lookback=6h) at time t
        # target_t = log((vol_6h_{t+6h} + eps) / (vol_6h_t + eps))
        ret_15m = raw_dfs["vwap"].pct_change()
        vol_6h = ret_15m.rolling(windows_6h, min_periods=windows_6h // 2).std()
        future_vol_6h = vol_6h.shift(-windows_6h)
        eps = 1e-8

        df_target = np.log((future_vol_6h + eps) / (vol_6h + eps))
        df_target = (
            winsorize(df_target.replace([np.inf, -np.inf], np.nan), 0.01, 0.99)
            .loc[keep_slice]
        )

        if self.use_market_neutral_target:
            available_market_symbols = [
                symbol for symbol in self.market_symbols if symbol in df_target.columns
            ]
            if available_market_symbols:
                market_target = df_target[available_market_symbols].mean(axis=1)
                df_target = df_target.sub(market_target, axis=0)
                for symbol in available_market_symbols:
                    df_target[symbol] = np.nan
                print(
                    f"Applied market-neutral residual target using {available_market_symbols}"
                )
            else:
                print(
                    f"Market symbols {self.market_symbols} not found in target columns; "
                    "skipping market-neutral residual"
                )

        print("Using target: forward 6-hour log volatility change")
