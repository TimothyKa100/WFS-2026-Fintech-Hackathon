...
def main():
    parser = argparse.ArgumentParser(
        description="Inference for models trained by 1 hour ahead.py"
    )
    ...
    factor_dfs = capture_factor_dfs_via_run(
        module_path=str(Path("1 hour ahead.py").resolve()),
        class_name="OptimizedModel",
        module_name="hour1_model_module",
    ...
    if pred.ndim != 1:
        raise RuntimeError("Expected regression output for 1-hour model")
