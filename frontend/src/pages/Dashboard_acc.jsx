...
function AssetModelOutputs({ rows = [] }) {
  if (!rows?.length) return null;

  const displayRows = [...rows]
    .map((r) => ({
      ...r,
      _label: r.label || r.id || r.symbol,
      _probUp: Number(r.prob_up ?? 0),
      _probDown: Number(r.prob_down ?? 0),
      _probNeutral: Number(r.prob_neutral ?? 0),
      _vol: Number(r.vol ?? 0),
  ...
  const classBadge = (row) => {
    const cls = String(row.class_label || "neutral").toLowerCase();
    const clsStyle =
      cls === "up"
    ...
    return (
      <span className={`text-[10px] px-2 py-0.5 rounded-full border ${clsStyle}`}>
        {cls}
      </span>
    );
  };
  ...
  return (
    <div className="mt-4 pt-4 border-t border-border">
      <div className="flex items-center justify-between">
        <div className="text-xs font-semibold text-text">Stock Model Outputs</div>
        <div className="text-[11px] text-muted">as-of 2026-02-28</div>
      </div>
      ...
      <div className="mt-3 space-y-2">
        {displayRows.map((r) => (
          <div
            key={`model-${r._label}`}
            className="rounded-2xl border border-border bg-bg/40 px-3 py-2"
          >
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-semibold text-text">{r._label}</div>
              <div className="flex items-center gap-2">
                {classBadge(r)}
                <span className="text-[11px] text-muted">
                  conf <span className="text-text font-semibold">{Math.round(r._confidence * 100)}%</span>
                </span>
              </div>
            </div>

            <div className="mt-1 text-[11px] text-muted flex flex-wrap items-center gap-x-3 gap-y-1">
              <span>
                up <span className="text-text font-semibold">{Math.round(r._probUp * 100)}%</span>
              </span>
              <span>
                neutral <span className="text-text font-semibold">{Math.round(r._probNeutral * 100)}%</span>
              </span>
              <span>
                down <span className="text-text font-semibold">{Math.round(r._probDown * 100)}%</span>
              </span>
              <span>
                vol <span className="text-text font-semibold">{(r._vol * 100).toFixed(1)}%</span>
              </span>
            </div>
          </div>
        ))}
...
export default function Dashboard() {
  ...
  const assetPredictions =
    forecast?.asset_predictions ?? forecast?.assetPredictions ?? [];
                ...
                <div className="relative">
                  <Panel title="System Risk" className="h-full">
                    <div className="space-y-3">
                      ...
                      <div className="rounded-2xl border border-border/60 bg-bg/30 p-3">
                        <AssetModelOutputs rows={assetPredictions} />
                      </div>
                    </div>
                  </Panel>
                </div>
